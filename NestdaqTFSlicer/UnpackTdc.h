/*
 * Modified UnpackTdc.h for NestdaqTFSlicer
 * Added 'inline' to all function definitions to prevent multiple definition errors.
 */
#ifndef UNPACK_TDC_H
#define UNPACK_TDC_H
#include <iostream>
...
inline void Rev5(unsigned char *val, unsigned char *rval)
{
	rval[4] = val[0];
	rval[3] = val[1];
	rval[2] = val[2];
...
inline int Unpack(unsigned char *data, struct tdc40 *tdc)
{
...
inline int Unpack(uint64_t data, struct tdc64 *tdc)
{
...
inline int Unpack(unsigned char *data, struct tdc64 *tdc)
{
	uint64_t *pdata = reinterpret_cast<uint64_t *>(data);
	return Unpack(*pdata, tdc);
}

inline int GetHBFrame(unsigned char *pdata, unsigned char *pend, unsigned char **ppnext)
{
...
inline int tdc64h_dump(uint64_t data)
{
